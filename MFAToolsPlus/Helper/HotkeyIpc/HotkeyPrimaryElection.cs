...
/// 使用全局互斥锁（不带目录路径），确保所有 MFAToolsPlus 实例中只有一个注册全局钩子
/// </summary>
public class HotkeyPrimaryElection : IDisposable
{
    private const string StateFileName = "hotkey_state.json";
    private const string PipeName = "MFAToolsPlus_HotkeyIpc";
    // 全局互斥锁名称 - 不包含目录路径，对所有 MFAToolsPlus 实例生效
    private const string GlobalMutexName = "Global\\MFAToolsPlus_HotkeyPrimary";
